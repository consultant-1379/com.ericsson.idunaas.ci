...
def aws_image_handler(args):
    ...
    utils.retry_func(partial(image_manager.__login_repo, args.aws_repo, args.aws_region), max_retry=10)
    image_manager.__check_create_repo(images, args.aws_repo, args.aws_region)
    config = configparser.ConfigParser()
    config.read(config_file)
    # exclude_list is a comma separated list of image names (without tag)
    exclude_list = config['IMAGES']['EXCLUDE'].split(',')
    for image in images:
        image_name = image.__str__().split('/')[-1].split(':')[0]
        if image_name in exclude_list:
            logging.info("Image {} skypped because it is in blacklist (exclude list)".format(image_name))
            continue
        image_manager.__pull_images(image)
        utils.retry_func(partial(image_manager.__push_images, image, args.aws_repo), max_retry=10)
