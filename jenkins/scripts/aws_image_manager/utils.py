...
def retry_func(func, max_retry=3):
    if max_retry < 1:
        return
    ...
    for retry in range(0, max_retry):
        try:
            ...
            logging.info("Call number {0}".format(retry + 1))
            logging.error('Failed execution, attempting again', exc_info=True)
            logging.debug('Addition information on the stack', stack_info=True)
            time.sleep(30)
    else:
