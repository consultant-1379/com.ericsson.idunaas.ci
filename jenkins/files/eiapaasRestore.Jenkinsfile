...
pipeline {
    ...
    parameters {
        ...
        string (
            name:           'BACKUP_SERVER',
            description:    'Backup server to store the backups externally'
        )
        string (
            name:           'BACKUP_SCOPE',
    ...
    environment {
        ...
        APPMGR_USER_CREDENTIALS = 'appmgr_user_credentials'
    ...
    stages {
        ...
        stage('Gather the backup name') {
            steps {
                sh "${bob_aas} get-backup-name"
            }
        }
        stage('Import secrets from minio') {
            steps {
                withCredentials([usernameColonPassword(credentialsId: env.MINIO_USER_SECRET, variable: 'MINIO_CREDS')]) {
                    sh "${bob_aas} import-minio-secrets:import-eic-secrets-from-minio-server"
                }
            }
        }
        ...
        stage ('Check backup --> Restore backup. All stages will be executed here using the credentials') {
            ...
            stages {
                stage('Check if backup is already in BRO'){
                    ...
                    steps{
                        sh "${bob_aas} check-backup-in-bro-interactive"
                    }
                }
                stage('Delete backup from BRO') {
                    ...
                    steps{
                        sh "${bob_aas} delete-backup-interactive"
                        sh "rm -f .delete_backup"
                    }
                }
                stage('Pre Restore - kms scaledown') {
                    steps{
                        sh "${bob_aas} kms-statefulset-wa:kms-statefulstate-scaledown"
                    }
                }
                stage('Import the backup') {
                    ...
                    steps{
                        sh "${bob_aas} import-restore-interactive:interactive-import"
                    }
                }
                stage('Restore the Backup') {
                    ...
                    steps {
                        sh "${bob_aas} pre-restoration-hook"
                        // temporary workaround: see IDUN-33220 and https://eteamproject.internal.ericsson.com/browse/ADPPRG-77657
                        sh "${bob_aas} apply-workaround-for-restore:apply-workaround"
                        sh "${bob_aas} import-restore-interactive:interactive-restore"
                        sh "${bob_aas} post-restore-secret-task:secret-creation"

                        retry(count: 10){
                            script{
                                if (RETRY_ATTEMPT > 1) {
                                    echo "Rerunning the \"Cleanup of rApps\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                                    sleep(30)
                                }
                                else {
                                    echo "Running the \"Cleanup of rApps\" stage. Try ${RETRY_ATTEMPT} of 5"
                                }
                                RETRY_ATTEMPT = RETRY_ATTEMPT + 1

                                withCredentials ([
                                    usernamePassword (
                                        credentialsId:      env.APPMGR_USER_CREDENTIALS,
                                        usernameVariable:   'APPMGR_USER',
                                        passwordVariable:   'APPMGR_PASSWORD'
                                    )
                                ]) {
                                    sh "${bob_aas} post-restore-rapps:cleanup-rapps"
                                }
                        ...
                        sh "${bob_aas} post-restore-pod-delete:pod-recreate"
                    }
                    post {
                        always {
                            sh "${bob_aas} post-restoration-hook"
                        }
                        success {
                            withCredentials ([
                                usernamePassword (
                                    credentialsId:      env.CASSANDRA_USER_SECRET,
                                    usernameVariable:   'CASSANDRA_USERNAME',
                                    passwordVariable:   'CASSANDRA_PASSWORD'
                                )
                            ]) {
                                sh "${bob_aas} uds-kafka-topics-delete"
                            }
                        }
                    }
                }
        ...
        stage('Remove ENM Subsystem') {
            steps {
                retry(count: 5){
                    script{
                        if (RETRY_ATTEMPT > 1) {
                            echo "Rerunning the \"Remove ENM subsystem\" stage. Retry ${RETRY_ATTEMPT} of 5. Sleeping before retry..."
                            sleep(30)
                        }
                        else {
                            echo "Running the \"Remove ENM subsystem\" stage. Try ${RETRY_ATTEMPT} of 5"
                        }
                        RETRY_ATTEMPT = RETRY_ATTEMPT + 1
                        ...
                        withCredentials([usernamePassword(credentialsId: env.IDUN_USER_SECRET, usernameVariable: 'IDUN_USER_USERNAME', passwordVariable: 'IDUN_USER_PASSWORD')]) {
                            sh "${bob_aas} remove-enm-subsystem"
                        }
                        RETRY_ATTEMPT = 1
                    }
                }
