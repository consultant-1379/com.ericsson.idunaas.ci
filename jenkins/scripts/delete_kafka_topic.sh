...
function check_empty_topic()
{
    ...
    kubectl --kubeconfig "${KUBE_CONFIG_PATH}" exec -t  eric-data-coordinator-zk-0 -n "${NAMESPACE}" -- \
...
function main()
{
   IS_UDS_CHART_DEPLOYED=$(helm -n "${NAMESPACE}" --kubeconfig $KUBE_CONFIG_PATH \
            ls -o json \
                | jq -r -c '.[] | select(.name=="eric-oss-uds")' \
                | wc -l)
   test ${PIPESTATUS[0]} -ne 0 \
    && echo "Error: Failed to run helm ls command" \
    && exit 1
   test $IS_UDS_CHART_DEPLOYED -eq 0  \
        && echo "Info: Deployment 'eric-oss-uds' helm chart is not deployed in this cluster.
        Nothing to do" \
        && exit 0
   declare -i count=1
   get_cassandra_key
