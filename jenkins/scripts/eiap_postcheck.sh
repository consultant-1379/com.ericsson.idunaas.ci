...
{
  output=$(kubectl get vs --kubeconfig "${KUBE_CONFIG}" -n "${NAME_SPACE}")
  if [[ $? != 0 ]]; then
	kubectl get vs --kubeconfig "${KUBE_CONFIG}" -n "${NAME_SPACE}" >>$LOG_FILE
...
{
  output=$(kubectl get vs --kubeconfig "${KUBE_CONFIG}" -n "${NAME_SPACE}" | grep gas${DOMAIN})
  if [[ $? != 0 ]]; then
        kubectl get vs --kubeconfig "${KUBE_CONFIG}" -n "${NAME_SPACE}"  >>$LOG_FILE
