...
pipeline {
    ...
    stages {
        ...
        */
        ...
        stage('Add cluster to mongodb') {
            steps {
                script {
                    sh "${bob} pooled-deployment:onboard-cluster-to-mongo"
                }
            }
        }
        stage('Create Namespaces in cluster for onboarding') {
            steps {
                script {
                    sh "${bob} pooled-deployment:create-cluster-namespaces-for-onboarding"
                }
            }
        }
        stage("Update cluster resource usage"){
            steps{
                script {
        ...
        stage('Perform Node Configurations to cluster') {
            steps {
                script {
                    sh "${bob} pooled-deployment:perform-node-config-to-cluster"
                }
            }
        }
