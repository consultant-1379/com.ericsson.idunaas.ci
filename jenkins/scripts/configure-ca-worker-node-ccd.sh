...
echo "INFO: Certificate will be dropped off to /etc/pki/trust/anchors/${CERTIFICATE_NAME}.crt on all cluster worker nodes"
...
data:
  config.sh: |
    echo "\${TRUSTED_CERT}" > /etc/pki/trust/anchors/${CERTIFICATE_NAME}.crt && update-ca-certificates && systemctl restart containerd
---
apiVersion: apps/v1
kind: DaemonSet
metadata:
