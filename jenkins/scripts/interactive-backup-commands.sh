...
if [[ $COMMAND == "backup export" ]] || [[ $COMMAND == "backup import" ]]; then
    #docker run -it --rm -u $(id -u):$(id -g) -v $PWD:/workdir --volume $PWD/aws:/.aws --volume /etc/hosts:/etc/hosts --volume /usr/local/bin:/usr/local/bin --volume /usr/local/aws-cli:/usr/local/aws-cli --workdir /workdir -e AWS_CONFIG_FILE=/workdir/aws/config -e AWS_SHARED_CREDENTIALS_FILE=/workdir/aws/credentials armdocker.rnd.ericsson.se/proj-eric-oss-drop/eric-oss-deployment-manager:latest $COMMAND -h "https://${GAS_HOSTNAME}" --name $BACKUP_NAME  -d "${BACKUP_SERVER}:22/${BACKUP_SERVER_REMOTE_PATH}/${ENV_NAME}" -v $DEBUG_MODE -n $NAMESPACE;
    kubectl --namespace $NAMESPACE exec -it --kubeconfig ./admin.conf deploymentmanager -- /venv/.venv/bin/python -m deployment_manager $COMMAND -h "https://${GAS_HOSTNAME}" --name $BACKUP_NAME --scope $BACKUP_SCOPE -d "${BACKUP_SERVER}:22/${BACKUP_SERVER_REMOTE_PATH}/${ENV_NAME}" -v $DEBUG_MODE -n $NAMESPACE;
fi;
...
if [[ $COMMAND == "backup create" ]] || [[ $COMMAND == "restore" ]] || [[ $COMMAND == "backup delete" ]]; then
    ...
    kubectl --namespace $NAMESPACE exec -it --kubeconfig ./admin.conf deploymentmanager -- /venv/.venv/bin/python -m deployment_manager $COMMAND -h "https://${GAS_HOSTNAME}" --name $BACKUP_NAME --scope $BACKUP_SCOPE -v $DEBUG_MODE -n $NAMESPACE;
fi;

if [[ $COMMAND == "backup view" ]]; then
    kubectl --namespace $NAMESPACE exec -it --kubeconfig ./admin.conf deploymentmanager -- /venv/.venv/bin/python -m deployment_manager $COMMAND -h "https://${GAS_HOSTNAME}" --scope $BACKUP_SCOPE -v $DEBUG_MODE -n $NAMESPACE;
