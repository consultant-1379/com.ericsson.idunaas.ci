...
pipeline {
    ...
    parameters {
        ...
        string(
            name:        'ENABLE_KUBEDOWNSCALER_UPDATE',
            description: 'Enable the update of Kubedownscaler. WARN: Remember to backup the configuration before update.',
            defaultValue: 'false'
        )
    }
    ...
    stages {
        ...
        stage('Upgrade') {
            when {
                not {
                    environment ignoreCase: true, name: 'ENABLE_KUBEDOWNSCALER_UPDATE', value: 'true'
                }
            }
            steps {
                sh "${bob} upgrade-cluster:upgrade-cluster"
            }
        }

        stage('Full Upgrade (including Kube-downscaler)') {
            when {
                environment ignoreCase: true, name: 'ENABLE_KUBEDOWNSCALER_UPDATE', value: 'true'
            }
            steps {
                sh "${bob} upgrade-cluster:upgrade-cluster-enabling-downscaler-update"
            }
        }
